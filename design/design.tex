...
\item
  ...
  respond in time. The proliferation of contacts and lines of
  communication is difficult to manage.
\item
  It is unclear what permits are needed, leading to lots of unnecessary
  back-and-forth between the department and applicants.
\item
...
\item
  Applicants are often unaware of what the different agencies need,
  leading to lots of to-ing and fro-ing to sort out the correct details.
\item
...
\item
  ...
  \item
    ...
    system to improve their experience in dealing with government for
    the activities they organise. They have identified a number of pain
    points which they would like to see improved. They see this project
    as part of a broader process of improving government services.
  \item
...
\item
  ...
  \item
    ...
    that the code would run on if the POC is converted to a full system.
    At this stage, none of the code runs on SSICT servers.
  \item
...
\item
  ...
  \item
    \emph{Communication Strategy}: These agency's interestes have been
...
\item
  ...
  systems that are unnecessary for a POC and are fiddly and time
  consuming have been put out of the scope.
\item
...
\item
  ...
  making it pretend that all transactions succeed without actually doing
  anything. This allows us to integrate payment steps into workflows,
...
\item
  ...
  the ANU provide us with networks to find experts to help us resolve
  any issues we are still stuck with.
\end{itemize}
...
The DigiACTive team is new, and therefore has all the usual risks of new
...
\item
  ...
  for DigiACTive to get help to correct problems as they arise.
\item
...
\item
  ...
  \$5000. This money is managed by NICTA.
\end{itemize}
...
for the booking of drivers' license tests, is tangentially related in
that it's a booking system, but proceeds from a much simpler model -
there is no need for multi-agency involvement, for example.
...
\item
  ...
  infrastructure with a minimum of fuss.
\end{itemize}

Before the POC can be deployed, future work will be required to bring it
in line with to following other standards:
...
\item
  ...
  developers to keep the system running.
\end{itemize}
